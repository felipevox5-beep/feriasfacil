...
const Dashboard: React.FC<DashboardProps> = ({ employees, vacations, onAskAI, onCancelVacation, onEditVacation, onUpdateVacation }) => {
  ...
  const pendingNotices = vacations
    .filter(v => parseLocalDate(v.startDate) > today && !v.noticeSent) // Only future and NOT sent
    .map(v => {
  ...
  const handleMarkAsSent = (vacation: Vacation) => {
    if (onUpdateVacation && confirm('Confirmar que o Aviso de Férias foi assinado?')) {
      onUpdateVacation({ ...vacation, noticeSent: true });
    }
  };
